...
@retry(wait=wait_random_exponential(min=5, max=60), stop=stop_after_attempt(max_attempt))
...
# For QA with embedding RPM
@retry(wait=wait_random_exponential(min=10, max=60), stop=stop_after_attempt(max_attempt))
